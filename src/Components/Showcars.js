...
const CarList = () => {
  ...
  useEffect(() => {
    ...
    const fetchCars = async () => {
      ...
      if (!token) {
       ...
       console.log(a);
      try {
        const response = await axios.get('http://localhost:8000/userCars', {
  ...
  const updateimage = async (carSID) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8000/updateCarImage/${carID}`,  
        formData,
        {
          headers: {
  ...
  console.log(carId);
    try {
      const response = await axios.put(
        `http://localhost:8000/updateCar/${carId}`,
        {
          title: editCar.title,
  ...
  const handleDeleteCar = async (carId) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:8000/deleteCar/${carId}`, {
        headers: { 'auth-token': token },
      });
  ...
  const handlePrevImage = () => {
    setCurrentImageIndex(
      (prev) => (prev - 1 + showMoreCar.images.length) % showMoreCar.images.length
    );
  };
    ...
    <div>
      ...
      {showMoreCar && (
        <div style={styles.modalBackdrop} onClick={handleCloseShowMoreModal}>
          ...
          >
            <h3 style={{ textAlign: 'left', color: 'black', margin: '0 0 10px 0' }}>Title: {showMoreCar.title}</h3>
            <p style={{ textAlign: 'left', color: 'black', margin: '0 0 10px 0' }}>Description: {showMoreCar.description}</p>
            <p style={{ textAlign: 'left', color: 'black', margin: '0 0 10px 0' }}>Tags: {showMoreCar.tags.join(', ')}</p>
      ...
      {isEditModalOpen && (
        <div style={styles.modalBackdrop} onClick={handleCloseEditModal}>
          ...
          >
            ...
            />
            <button
              onClick={() => handleUpdateCar(update_id)}
