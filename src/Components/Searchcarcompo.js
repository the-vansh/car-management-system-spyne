...
const CarList = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:8000/searchCars?query=${searchQuery}`, {
        headers: { 'auth-token': token },
      });
  ...
  const handlePrevImage = () => {
    setCurrentImageIndex(
      (prev) => (prev - 1 + selectedCar.images.length) % selectedCar.images.length
    );
  };
      ...
      {isModalOpen && selectedCar && (
        <div style={styles.modalBackdrop} onClick={handleCloseModal}>
          ...
          >
            ...
            <h3 style={{ textAlign: 'left', color: 'black', margin: '0 0 10px 0' }}>Title: {selectedCar.title}</h3>
            <p style={{ textAlign: 'left', color: 'black', margin: '0 0 10px 0' }}>Description: {selectedCar.description}</p>
            <p style={{ textAlign: 'left', color: 'black', margin: '0 0 10px 0' }}>Tags: {selectedCar.tags.join(', ')}</p>
